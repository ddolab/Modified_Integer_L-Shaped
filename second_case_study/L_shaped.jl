...
ISP_optgaps=[0.00]
time_lim_initial=RMP_time_limit
        ...
        current_ISP_index=findfirst(all(j .- 1e-8 .<= X_val .<= j .+ 1e-8) for j in X_list_int_sol)
            ...
            while num_of_solutions_for_ISP[current_ISP_index] < length(ISP_optgaps)
                integer_SP_time = @timed begin
                    results=let problem_data= (theta_,D,I,L_i,J,K,H,θ_max,T_h,M,Cmax,C_max,Cnameplate,C_nameplate,J_,J0,epsilon,rho,eta,Bmax,M_i,CCmin,CCmax,TR_i,SQ_i,Delta,bigM,theta,n,delta,gamma,phi,psi,p_s)
                        wp = CachingPool(workers())
                        pmap(s->do_ISP_optimization(s,problem_data,ISP_optgaps,num_of_solutions_for_ISP,current_ISP_index,RMP_time_limit,currenttime,ISP_solutions[s],X_val),wp,S_sp; retry_delays= ExponentialBackOff(n = 3)) 
                    end
                    results=Dict(s=>results[s] for s in S_sp)
                    ...
                    for s in S_sp
                        ...
                        if results[s][3] == TIME_LIMIT
                            con= @build_constraint(
                                    η[s] <= (η_val[s]-1e-2))
                            MOI.submit(RMP, MOI.LazyConstraint(cb_data), con)
                            print("terminated MISP")
                            #rmprocs(workers())
                            flush(stdout)
                            #GRBterminate(backend(RMP))
                            return
...
@everywhere function do_ISP_optimization(s,problem_data,ISP_optgaps,num_of_solutions_for_ISP,current_ISP_index,RMP_time_limit,currenttime,previous_solution_for_this_problem,X_val)
    (theta_,D,I,L_i,J,K,H,θ_max,T_h,M,Cmax,C_max,Cnameplate,C_nameplate,J_,J0,epsilon,rho,eta,Bmax,M_i,CCmin,CCmax,TR_i,SQ_i,Delta,bigM,theta,n,delta,gamma,phi,psi,p_s)=problem_data
   ...
   if currenttime > RMP_time_limit + 100
        return [0,0,TIME_LIMIT,0]
   else
        set_optimizer_attribute(ISP_model_s, "TimeLimit", 101+RMP_time_limit-currenttime)
   end
    ...
    if termination_status(ISP_model_s)==TIME_LIMIT 
        return [0,0,TIME_LIMIT,ISP_s_time]
    end
